...
// Encode generates the experiment for the standalone generator, i.e. with no
// service setup to report.
//
// The setup argument must be nil, not false: EncodeWithContext branches on
// `setup != nil`, and a false stored in an interface{} is not nil, so passing
// false made this emit "Generated by orchestrator service with setup: false".
func Encode(p *GenParams, writeCommand func(GeneratedCommand), writeComment func(string)) ExperimentInfo {
	return EncodeWithContext(p, writeCommand, writeComment, nil)
}
...
func EncodeWithContext(p *GenParams, writeCommand func(GeneratedCommand), writeComment func(string), setup interface{}) ExperimentInfo {
	...
	if setup != nil {
		// Redaction is done here rather than at the call sites: this comment
		// is printed through StoreLogging into ExperimentState.Logs, which
		// GET /api/v0/experiment/status serves without authentication, so a
		// setup carrying a credential must never be marshalled raw. Doing it
		// here means a new caller cannot forget.
		if r, ok := setup.(interface{ RedactedForLog() any }); ok {
			setup = r.RedactedForLog()
		}
	...
	fundCmds := []FundParams{}
	// Not `var rounds []RoundInfo`: at rounds = 0 that marshals to null, and a
	// consumer reading result.rounds.length gets a TypeError rather than 0.
	rounds := []RoundInfo{}
